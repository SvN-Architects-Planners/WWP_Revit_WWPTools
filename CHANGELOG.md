...
### Fixed
- Family Swapper: added "Set target" button next to the target family dropdown; clicking it discovers parameters without requiring a placed instance of the target family -- type parameters are read directly from the `FamilySymbol`, and instance parameters are read from any placed titleblock (project/shared parameters apply to all titleblock families equally). Previously the target parameter column was incorrectly showing source family parameters because `Discover` matched target types by name across all families rather than by family. Parameter discovery and run now also include type parameters in addition to instance parameters; the run step falls back to writing type parameters when the target instance parameter is not found.
