...
- Telemetry: renamed `command-exec.py` → `journal-command-exec.py` — pyRevit only recognises `journal-command-exec` as the hook for any journaled command; the old name never fired. Rewrote the hook to filter by `EXEC_PARAMS.command_extension` instead of `event_args.CommandId.Name` (which is a Revit built-in command ID, not the script name).
- `#! python3` engine directive moved to line 1 in 9 additional scripts (`Add Line Type`, `SetupLevel`, `CreateViewsFromLevel`, `DuplicateViewsForLevel`, `FireRatingAll`, `FireRatingSelected`, `FireRatingFRRViews`, `DoorFireRating`, `CombinedPrintSet`) that still had the coding comment on line 1, preventing pyRevit 6.4 from routing them to CPython.
