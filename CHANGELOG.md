...
- Telemetry: fixed `UnicodeEncodeError` crash in `app-init.py` when writing to `script_log.jsonl` — IronPython 2.7's `open()` defaults to ASCII encoding, which can't encode emoji characters generated by `_format_details_md`. Now uses `io.open` with explicit UTF-8 encoding so previous-session records are correctly written and processed files are moved to `_processed_telemetry`.
