...
- `app-init.py` startup toast and JSONL logging broken on pyRevit 6.4: the telemetry-config block used `except Exception` which does not catch `SystemExit` — pyRevit 6.4 can raise `SystemExit` from `save_changes()` or the telemetry API, silently killing the rest of the hook before the toast ran. Changed all three `except Exception` guards in the telemetry block to `except BaseException`.
